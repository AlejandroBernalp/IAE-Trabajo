...
def cargar_datos_desde_r():
    ...
    datos <- read.table(url, header = FALSE, sep = " ")
    ...
    datos <- datos %>% mutate(
        checking_status = case_when(checking_status == "A11" ~ "< 0 DM", checking_status == "A12" ~ "0-200 DM", checking_status == "A13" ~ ">= 200 DM", checking_status == "A14" ~ "no checking"),
        credit_history = case_when(credit_history == "A30" ~ "no credits", credit_history == "A31" ~ "all paid duly", credit_history == "A32" ~ "existing paid", credit_history == "A33" ~ "past delay", credit_history == "A34" ~ "critical account"),
        purpose = case_when(purpose == "A40" ~ "car (new)", purpose == "A41" ~ "car (used)", purpose == "A42" ~ "furniture/equipment", purpose == "A43" ~ "radio/television", purpose == "A44" ~ "domestic appliances", purpose == "A45" ~ "repairs", purpose == "A46" ~ "education", purpose == "A47" ~ "vacation", purpose == "A48" ~ "retraining", purpose == "A49" ~ "business", purpose == "A410" ~ "others"),
        savings_status = case_when(savings_status == "A61" ~ "< 100 DM", savings_status == "A62" ~ "100-500 DM", savings_status == "A63" ~ "500-1000 DM", savings_status == "A64" ~ ">= 1000 DM", savings_status == "A65" ~ "no savings"),
        employment_since = case_when(employment_since == "A71" ~ "unemployed", employment_since == "A72" ~ "< 1 year", employment_since == "A73" ~ "1-4 years", employment_since == "A74" ~ "4-7 years", employment_since == "A75" ~ ">= 7 years"),
        personal_status = case_when(personal_status == "A91" ~ "male: divorced/sep", personal_status == "A92" ~ "female: div/sep/mar", personal_status == "A93" ~ "male: single", personal_status == "A94" ~ "male: mar/wid", personal_status == "A95" ~ "female: single"),
        other_debtors = case_when(other_debtors == "A101" ~ "none", other_debtors == "A102" ~ "co-applicant", other_debtors == "A103" ~ "guarantor"),
        property_type = case_when(property_type == "A121" ~ "real estate", property_type == "A122" ~ "life insurance", property_type == "A123" ~ "car/other", property_type == "A124" ~ "no property"),
        installment_plans = case_when(installment_plans == "A141" ~ "bank", installment_plans == "A142" ~ "stores", installment_plans == "A143" ~ "none"),
        housing_type = case_when(housing_type == "A151" ~ "rent", housing_type == "A152" ~ "own", housing_type == "A153" ~ "for free"),
        job_type = case_when(job_type == "A171" ~ "unskilled non-res", job_type == "A172" ~ "unskilled res", job_type == "A173" ~ "skilled official", job_type == "A174" ~ "mgmt/highly qualif"),
        telephone = case_when(telephone == "A191" ~ "none", telephone == "A192" ~ "yes"),
        foreign_worker = case_when(foreign_worker == "A201" ~ "yes", foreign_worker == "A202" ~ "no")
    )
